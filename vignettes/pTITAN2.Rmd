...
```{r }
c All other name spaces are used explicitly.haparral_envgrad_normal <-
...
to individual processing cores.  Parallel computing is platform specific.  An
example script for how to run `TITAN2::titan` as a parallel process can be found
in the provided example script:

```{r}
# file.open(system.file())
```
