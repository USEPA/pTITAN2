  ...
  replicate(4,
            {
              p <- permute(taxa = list(CD_06_Mall_wID, CN_06_Mall_wID),
                           ...
                           sid = "StationID")
              trt_codes_1 <- occurrences(p$Treatment1$taxa, n = 6L)$taxon
              trt_codes_2 <- occurrences(p$Treatment2$taxa, n = 6L)$taxon
              ...
              out1 <- try(
                          TITAN2::titan(env  = p[["Treatment1"]][["env"]][[1]],
                                        ...
                                        boot = TRUE,
                                        nBoot = 5)
                          , #Change the number of bootstraps here
                          silent = TRUE)
              out2 <- try(TITAN2::titan(env  = p[["Treatment2"]][["env"]][[1]],
                                        txa  = trt_codes_2,
                                        boot = TRUE,
                                        nBoot = 5),
                          silent = TRUE)
